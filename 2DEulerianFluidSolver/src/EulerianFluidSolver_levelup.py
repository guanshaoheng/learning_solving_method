...
def main():
    ...
    rho = 1. + (np.abs(Y - 0.5) < 0.25); plt.imshow(rho.T);plt.colorbar(); plt.show()
    vx = -0.5 + (np.abs(Y - 0.5) < 0.25) ; plt.imshow(vx.T);plt.colorbar(); plt.show()
    vy = w0 * np.sin(4 * np.pi * X) * (
                np.exp(-(Y - 0.25) ** 2 / (2 * sigma ** 2)) + np.exp(-(Y - 0.75) ** 2 / (2 * sigma ** 2)))
    plt.imshow(vy.T);plt.colorbar();plt.show()
