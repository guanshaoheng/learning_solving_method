...
'''
    Reference: https://forum.taichi-lang.cn/t/topic/3883/4
'''
...
triangles = ti.Vector.field(3, ti.i32, N_triangles)  # here saved the node index of the triangles
edges = ti.Vector.field(2, ti.i32, N_edges)

# operator in computation
...
def compute_D(i):
    '''
        Get the (node[2] - node[1], node[3] - node[1])
    '''
    a = triangles[i][0]
    b = triangles[i][1]
    c = triangles[i][2]
...
def compute_gradient():
    ...
    for i in range(N_triangles):
        Ds = compute_D(i)
        F = Ds @ elements_Dm_inv[i]   # where F is the deformation gradient
...
while gui.running:
    ...
    gui.text(
        content='Auto_diff: %s' % using_auto_diff, pos=(0.6, 0.825), color=0xFFFFFF)
    gui.show()
