...
def write_down_infor(
        ...
        energy: float, energy_potential: float, energy_contact: float, fx:float = None, fy:float=None):
    save_dir_name = os.path.join(save_dir_name, 'laod_infor')
    ...
    if fx is None:
        ...
        header = 'x y radius \n E:%.2e E_potential:%.2e E_contact:%.2e fx:%.2e fy:%.2e' % (
        energy, energy_potential, energy_contact, fx, fy)
    np.savetxt(
...
def overlap_check(x: torch.Tensor, r: torch.Tensor):
    ...
    distance = torch.sqrt(x0_diff ** 2 + x1_diff ** 2 + 1e-16) + torch.eye(len(x), device=device) * r_0_1
    overlap = r_0_1 - distance
...
def plot_particles(xx: np.ndarray, r: np.ndarray, step: int,
    ...
    if top_boundary is not None:
        plt.plot([0, 11], [top_boundary, top_boundary], linewidth=3, c='k')
    if bottom_boundary is None:
        plt.plot([0, 11], [0, 0], linewidth=3, c='k')
    else:
        plt.plot([0, 11], [bottom_boundary, bottom_boundary], linewidth=3, c='k')
    plt.axis('equal')
    # plt.axis('off')
    if step == 0:
...
def plot_E_history(E, E_potential, E_contact, save_dir_name):
    ...
    plt.tight_layout()
    if save_dir_name:
        plt.savefig(os.path.join(save_dir_name, 'hitsory.png'), dpi=200)
    else:
        plt.show()
    plt.close()
