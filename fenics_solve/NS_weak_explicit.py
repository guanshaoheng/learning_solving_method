...
def main():
    ...
    for t in tqdm(range(N_TIME_STEPS)):
        ...
        """
        # 如果采用一下方式求解，则会报“Solving linear variational problem.”
        # 这不是错误，但是会导致计算效率降低
        # 因为求解的方程的时候左手边的方程对应的全局矩阵并未变化，所以反复重新计算，可以在进入循环前组装该矩阵
        fe.solve(
            momentum_weak_form_lhs == momentum_weak_form_rhs,
            u_tent,
